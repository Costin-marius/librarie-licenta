...
function DetaliiCarte({ cos, setCos, wishlist, setWishlist, userId }) {
    ...
    // 1. Efectul principal (AICI AM FĂCUT MODIFICAREA)
    useEffect(() => {
        window.scrollTo(0, 0);
        setLoading(true);
        // Resetăm ratingul vizibil al utilizatorului când se schimbă cartea (ID-ul din URL)
        setUserRating(0); 
        ...
        fetch(`http://localhost:5000/api/carti/${id}`)
            ...
            .then(data => {
                ...
                // Luăm ID-ul curent (fie din props, fie din localStorage)
                const idUtilizatorCurent = userId || localStorage.getItem('userId');

                // Verificăm dacă userul a votat deja ACEASTĂ carte
                if (idUtilizatorCurent && data.ratinguri && Array.isArray(data.ratinguri)) {
                    // Folosim .toString() pentru a ne asigura că tipurile de date se potrivesc
                    const ratingExistent = data.ratinguri.find(
                        r => r.utilizator && r.utilizator.toString() === idUtilizatorCurent.toString()
                    );
                    
                    if (ratingExistent) {
                        setUserRating(ratingExistent.nota);
                    } else {
                        setUserRating(0); // Dacă nu a votat, rămâne 0
                    }
                }
            })
            .catch(error => {
    ...
    // 2. Efectul secundar (Recomandări)
    useEffect(() => {
                ...
                <div className="flex flex-col md:flex-row p-6 md:p-8 gap-8">
                    ...
                    <div className="w-full md:w-1/4 flex flex-col items-center">
                        <img
                            ...
                            alt={carte.titlu}
                            className="w-full max-w-[200px] aspect-[2/3] object-cover rounded shadow-xl border border-gray-200 dark:border-gray-700"
                        />
                    </div>
