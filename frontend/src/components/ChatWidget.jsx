...
const ChatWidget = ({ cos, setCos }) => { 
  const navigate = useNavigate(); // <-- Inițializat hook-ul de navigare
  ...
  const handleSendMessage = async (e) => { 
    ...
    try { 
      ...
      setMessages(prev => [...prev, { 
        role: 'assistant', 
        content: '', 
        type: data.carti_recomandate && data.carti_recomandate.length > 0 ? 'recommendation' : 'text', 
        books: data.carti_recomandate || null, 
        buton_navigare: data.buton_navigare || null 
      }]); 
      
      let assistantMessageContent = ''; 
      let charQueue = data.mesaj ? data.mesaj.split('') : []; 
      ...
      const startTypewriter = () => { 
        ...
        typewriterInterval = setInterval(() => { 
          ...
          if (charQueue.length > 0) { 
            const chars = charQueue.splice(0, 1); 
            assistantMessageContent += chars[0]; 
            setMessages(prev => { 
              const updated = [...prev]; 
            ...
            if (data.actiune_cos && data.carti_recomandate) { 
              const bookToAdd = data.carti_recomandate.find(b => b._id.toString() === data.actiune_cos); 
              if (bookToAdd) handleAdaugaInCos(bookToAdd, 1); 
            } 
          } 
        }, 10); 
      }; 
      ...
      if (axios.isCancel(error) || error.name === 'AbortError' || error.message?.includes('canceled')) { 
        ...
        <div className="absolute bottom-20 right-0 w-[350px] sm:w-[400px] h-[550px] max-h-[80vh] bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-stone-200 dark:border-slate-700 flex flex-col overflow-hidden animate-fade-in-up"> 
          ...
          <div className="bg-amber-500 dark:bg-amber-600 p-4 text-white flex items-center justify-between"> 
            <div className="flex items-center gap-3"> 
              <div className="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center font-bold text-lg"> B </div> 
              <div> 
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-stone-50 dark:bg-slate-800/50 scrollbar-hide"> 
            {messages.map((msg, idx) => ( 
              <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}> 
                <div className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm flex flex-col gap-3 ${ msg.role === 'user' ? 'bg-amber-500 text-white rounded-br-none' : 'bg-white dark:bg-slate-800 text-stone-700 dark:text-stone-300 border border-stone-100 dark:border-slate-700 rounded-bl-none shadow-sm' }`}> 
                  <div>{msg.content}</div> 
                    ...
                    <div className="flex flex-col gap-5 mt-2 border-t border-stone-100 dark:border-slate-700 pt-3"> 
                      {msg.books.map(book => ( 
                        <div key={book._id} className="flex flex-col gap-3"> 
                          {/* Cardul propriu-zis cu onClick pentru Navigare */} 
                          <div 
                            onClick={() => navigate(`/carte/${book._id}`)} 
                            className="flex gap-3 items-center bg-stone-50 dark:bg-slate-900 border border-stone-200 dark:border-slate-700 rounded-xl p-2 hover:shadow-md transition-all cursor-pointer hover:bg-stone-100 dark:hover:bg-slate-800"
                          > 
                            ...
                            <button 
                              onClick={(e) => {
                                e.stopPropagation(); // Oprește navigarea dacă a dat click pe "Adaugă în coș"
                                handleAdaugaInCos(book);
                              }} 
                              className="w-9 h-9 rounded-full bg-amber-100 hover:bg-amber-500 text-amber-600 hover:text-white dark:bg-amber-900/30 dark:hover:bg-amber-500 transition-colors flex items-center justify-center shrink-0 shadow-sm z-10" 
                              title="Adaugă în coș" 
                            > 
                  ...
                  {/* Buton de navigare inteligentă */} 
                  {msg.buton_navigare && ( 
                    <div className="mt-2 text-center border-t border-stone-100 dark:border-slate-700 pt-3"> 
                      <a href={msg.buton_navigare.link} className="inline-block w-full px-4 py-2 bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 font-bold rounded-lg hover:bg-amber-200 dark:hover:bg-amber-900/50 transition-colors text-xs shadow-sm" > 
                        {msg.buton_navigare.text} 
                      </a> 
                    </div> 
                  )} 
                </div> 
          ...
          <form onSubmit={handleSendMessage} className="p-3 border-t border-stone-200 dark:border-slate-700 bg-white dark:bg-slate-900 flex items-center gap-2"> 
            <input 
              type="text" 
              className="flex-1 bg-stone-100 dark:bg-slate-800 border-none rounded-full px-4 py-2.5 focus:ring-2 focus:ring-amber-500/50 text-sm text-stone-700 dark:text-stone-200 outline-none" 
